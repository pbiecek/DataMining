...
Not that you may increase the number of classes to the sample size and this leads to one-leave-out validation. See `method="LOOCV"` for more details.
...
```{r}
model <- train(quality~., data=winesb, trControl=train_control, method="nb")
varImp(model, sacle=TRUE)
plot(varImp(model))
```

See here: http://jackaljack.github.io/wine-classification-with-R/ for interesting discussion related to feature selection.
...
5. Calculate performance (forest/knn/naiveBayes) for new dataset `letters`. Not that we have more than two classes here. 
