...
ffit3 <- randomForest(income ~ .,   data=trainSet, importance = TRUE, mtry=7)
...
#knn
ffit4 <- knn3(income ~ .,   data=trainSet, k=10)
prob4 <- predict(ffit4, newdata = testSet, type="prob")[,2]

ffit5 <- knn3(income ~ .,   data=trainSet, k=20)
prob5 <- predict(ffit5, newdata = testSet, type="prob")[,2]

ffit6 <- knn3(income ~ .,   data=trainSet, k=30)
