...
The Dataset is about interaction of stations in a museum by various vistors with entering and leaving time.
Our goal is to characterize the flow of visitors through these stations. 
We are trying to answer the following questions in Phase I
...
The caracterization of clusters is made by plotting variables used in the clustering and the resulting clusters. So the above plot help us to define the caracteristic of each cluster:  

1.Dance, Example, Game over corresponding to the first cluster and it correspond to the **cnk04** machine who is believed to be a station with dancing patterns. This machine have a really small amount of time of interaction and can be described suitable for occasional players.  

2.Stations **cnk02a** and **cnk02b** are believed to be typical station machines, almost all users use these two stations who are believed to be regular stations at the museum that everyone uses. The time of interaction is quite short (<5min), although, both stations are used intensivly.

3.Station **cnk03** belong to the 4th cluster, and the most pattern used in this station are Multi level Main (LevelOneMain, LevelTwoMain,etc.). According to the boxplot of time interaction, this station is used with a great amount of time of interaction and may contain multilevel stages to accomplish the goal behind it. So users spend too much time with it and can be decribed as 'addictive' for visitors.  

The most used patterns are  more related to the stations **cnk02a** and **cnk02b**. As said before those patterns are Screen and Splash. This station is believed to have 
